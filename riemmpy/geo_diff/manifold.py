import sympy as sp
from sympy import diff, MutableDenseNDimArray
from scipy.integrate import solve_ivp
import numpy as np


class Manifold:
    def __init__(self, metric, coordinates, *, name="M"):
        """
    ...
    def __str__(self):
        ...
        manifold_str = f"{self.name}{to_superscript(self.dimension)}" if self.dimension<10 else f"{self.name}{self.dimension}"
        s = f"\n << Let {manifold_str} be a {self.dimension}-dimensional manifold of coordinates {self.coords} and metric tensor `{self.metric}` >>"
        # s += f"metric: {repr(self.metric)}\n"  # compact
        # mostra solo cosa è calcolato
        # computed = [name for name in [
        #     "Christoffel symbols", "Riemann tensor", "Ricci tensor",
        #     "Covariant Riemann", "Scalar curvature", "Einstein tensor",
        #     "Sectional curvatures", "Geodesics", "Kretschmann scalar"
        # ] if getattr(self, name.lower().replace(" ", "_")) is not None]
        # if computed:
        #     s += f"computed: {', '.join(computed)}"
        return s

    # Developer-friendly, mostra tutti gli attributi
    def __repr__(self):
